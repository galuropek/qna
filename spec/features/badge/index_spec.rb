...
feature 'User can view a achieved badges', %q{
  ...
  given!(:badge_one) { create(:badge, name: 'Superman', question: question_one, user: answer_author) }

  given(:question_two_user) { create(:user) }
  given(:question_two) { create(:question, user: question_two_user) }
  given!(:badge_two) { create(:badge, name: 'Batman', question: question_two, user: answer_author) }
  ...
  scenario 'User views a achieved badges list' do
    visit user_badges_path(answer_author)
